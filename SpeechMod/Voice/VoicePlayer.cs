...
{
    private static async Task PlayStream(Stream stream, string key, bool play = true)
    {
        using var waveOut = Main.WaveOutEvent;

        if (waveOut.PlaybackState == PlaybackState.Playing && play)
            waveOut.Stop();
    ...
    public static async Task PlayText(string text, string key, Gender gender, string speaker)
    {
        ...
        {
            var (finalName, textForVoice) = await TextParser.MakeTextForVoice(text, speaker);
            ...
            if (Main.VoiceSettings.SaveHighQuality && Main.VoiceSettings.Model != Constants.HighQualityModel)
            {
                ElevenReq highQualityReq = new()
                {
                    ModelID = Constants.HighQualityModel,
                    Text = textForVoice,
                    Voice = voice
                };

                var highQualityStream = await ElevenLabsGateway.CreateStream(highQualityReq);

                if (highQualityStream != null)
                    await PlayStream(highQualityStream, $"{key}", false);
            }
        }
    }
}